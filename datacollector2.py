...
for regatta_data in regatta_result_set:
    ...
    week = regatta_data[0]
    if("team" not in regatta_data[5].lower() and "single" not in regatta_data[5].lower() and "Official" == regatta_data[-2] and "jv" not in regatta_data[1].lower()
    and ("fundamental" in regatta_data[4].lower() or "conference" in regatta_data[4].lower() or "cross regional" in regatta_data[4].lower() or "national" in regatta_data[4].lower()) and ("21" in year or "22" in year or "23" in year or "24" in year)):
        print(regatta)
        print(year)
        regatta_set = []
        sailor_index = []
