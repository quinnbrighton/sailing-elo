...
df3 = load_csv('./data/coefs_output2.csv')
df3.columns = ['id'] + list(df3.columns[1:])
df1['id'] = df1['id'].astype('int64')
df3['id'] = df3['id'].astype('int64')
df2['id'] = df2['id'].astype('int64')


df4 = merge_dataframes(df2,df3,on='id', how='inner')
df5 = merge_dataframes(df1,df3,on='id', how='inner')

#df4 = load_csv('./data/skippers.csv')

#df5 = merge_dataframes(df3,df4,on='id', how='inner')

df_sorted_desc = df4.sort_values(by="Estimate", ascending=False)

print(df_sorted_desc)

df_sorted_desc.to_csv("data/skipperranks/f24w10.csv", index=False)
...
df_sorted_desc.to_csv("data/crewrank.csv", index=False)
